<?php 
	...
	if (isset($_POST['liked'])) {
		...
		mysqli_query($con, "INSERT INTO likes (userid, postid) VALUES (3, $postid)");
	...
	if (isset($_POST['unliked'])) {
		...
		mysqli_query($con, "DELETE FROM likes WHERE postid=$postid AND userid=3");
		mysqli_query($con, "UPDATE posts SET likes=$n-1 WHERE id=$postid");
		...
		<?php while ($row = mysqli_fetch_array($posts)) { ?>
			...
			<div class="post">
				...
				<?php 
					...
					$results = mysqli_query($con, "SELECT * FROM likes WHERE userid=3 AND postid=".$row['id']."");
